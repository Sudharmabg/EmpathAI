...
import com.mymercurie.user.dto.school.ClassSummaryResponse;
import com.mymercurie.user.dto.school.SchoolRequest;
import com.mymercurie.user.dto.school.SchoolResponse;
import com.mymercurie.user.dto.school.SchoolSummaryResponse;
import com.mymercurie.user.dto.user.StudentDetailResponse;
...
public class SchoolService {
    ...
    private void validateSchoolAccess(Long requestedSchoolId) {
        ...
        if (user instanceof com.mymercurie.user.entity.SchoolAdmin sa) {
            if (sa.getSchoolId() == null || !sa.getSchoolId().equals(requestedSchoolId)) {
                throw new MyMercurieException("Access denied: You can only access data for your assigned school", "ACCESS_DENIED");
            }
        } else if (user instanceof com.mymercurie.user.entity.Teacher t) {
            if (t.getSchoolId() == null || !t.getSchoolId().equals(requestedSchoolId)) {
                throw new MyMercurieException("Access denied: You can only access data for your assigned school", "ACCESS_DENIED");
            }
        } else if (user instanceof com.mymercurie.user.entity.Student s) {
    ...
    public SchoolResponse createSchool(SchoolRequest request) {
        if (schoolRepository.existsByName(request.getName())) {
            throw new MyMercurieException("School with name '" + request.getName() + "' already exists", "DUPLICATE_SCHOOL");
        }
    ...
    public List<SchoolSummaryResponse> getAllSchoolSummaries() {
        ...
        return schoolRepository.findAll().stream()
                .filter(school -> {
                    if (user instanceof com.mymercurie.user.entity.SchoolAdmin sa) return school.getId().equals(sa.getSchoolId());
                    if (user instanceof com.mymercurie.user.entity.Teacher t) return school.getId().equals(t.getSchoolId());
                    if (user instanceof com.mymercurie.user.entity.Student s) return school.getId().equals(s.getSchoolId());
                    return true;
                })
    ...
    public List<ClassSummaryResponse> getClassesBySchool(Long schoolId) {
        ...
        schoolRepository.findById(schoolId)
                .orElseThrow(() -> new MyMercurieException("School not found with id: " + schoolId));
    ...
    public SchoolResponse getSchoolById(Long id) {
        ...
        School school = schoolRepository.findById(id)
                .orElseThrow(() -> new MyMercurieException("School not found with id: " + id));
    ...
    public void deleteSchool(Long id) {
        ...
        if (!schoolRepository.existsById(id)) {
            throw new MyMercurieException("School not found");
        }
